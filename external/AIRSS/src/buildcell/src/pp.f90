...
contains
  ...
  subroutine eval_pp(e,f,s)
    ...
    integer       :: ion_i,ion_j,ion_k,ispeci,ispecj,ispeck
    integer       :: na,nna,nb,nnb,nc,nnc,ni,ns,nn,nnmx,nn0,n,m,mm,i,j

    integer, allocatable :: lnvec(:,:)

    real(kind=dp) :: radsph,R0(3),V0(3),ha,hb,hc,acb(3),acc(3),ccb(3),dec(num_ions*num_symm),ecoord,ethree,erep
    ...
    real(kind=dp) :: mod_r123,swork(3,3),s_symm(3,3),symm_count(3)
    real(kind=dp) :: rijsq,rijmod,mod_f123,mod_r13,mod_r23,riksq,rikmod,rjksq,rjkmod
    real(kind=dp) :: normal(3),widthby2,origin(3),distpln,distsph,dtmp,ftmp
    ...
    if(present(s).and.(.not.present(f))) stop 'eval_pp: forces must be computed to compute stress'
    ...
    allocate(lvec(3,nnmx),lnvec(3,nnmx),ll(nnmx))    
...
!!$                if(present(s))  s=s-outer_product(f12,real(lnvec(:,nn),dp))
...
!!$    if((num_symm.gt.1).and.present(f)) then
    ...
    if((num_symm.gt.1).and.present(s)) then
              
       swork=0.0_dp
       do ns=1,num_symm
          swork(:,:) = swork(:,:) + matmul(invert3x3(symm_ops(1:3,1:3,ns)),matmul(s(:,:),symm_ops(1:3,1:3,ns)))/real(num_symm,dp)
       end do
       s=swork
