...
222    ang = -1.0_dp
       do while(ang<0.0_dp)
          ...
          if(any(cang==99999.0_dp)) then
             ...
             if(internalsymm) then
                lattice_abc(1:3)=matmul(setting_perm,lattice_abc(1:3))
                lattice_abc(3+1:3+3)=matmul(setting_perm,lattice_abc(3+1:3+3))
             end if
       ...
       if(num_pairs.eq.0) then
          ...
          do n=1,num_spec
             do m=n,num_spec
                ...
                rn = random_triple()
                if(minsep.gt.-delta) then
                   pair_minsep(num_pairs)=minmin+rn(1)*(minmax-minmin)
                else
                   if(.not.any(posamp_set.ge.0.0_dp)) & ! ** Only set to default if POSAMP is not being used
                        pair_minsep(num_pairs)=elements_radius(get_z(species_names(n)))+elements_radius(get_z(species_names(m)))
                end if
             end do
          end do
       end if
...
!!$    write (stderr,*) num_ions,num_ions_total
                ...
                do m=1,nc3
                   ...
                   if(m.le.ion_coord(1,ni)) then
                      ...
                      if(dist2.le.dminang2) then
                         ion_fails(ni)=ion_fails(ni)+1
                         if(dist2.gt.delta) then
                            v12=-vep(:)*sqrt(dminang2-dist2)/2.0_dp/sqrt(dist2)
                         else
                            v12=0.0_dp
                         end if
                      ...
                      if(dist2.ge.dmaxang2) then
                         ion_fails(ni)=ion_fails(ni)+1
                         if(dist2.gt.delta) then
                            v12=vep(:)*sqrt(dist2-dminang2)/2.0_dp/sqrt(dist2)
                         else
                            v12=0.0_dp
                         end if
                         if(ion_bfix(nj)) v12=0.0_dp
  ...
  subroutine assign_spin(stat)
    ...
    if(sum(abs(spinvec)).gt.0.0_dp) return
